...
const EditCandidate = () => {
    ...
    const fetchCandidate = async () => {
      ...
      try {
        const response = await fetch(
          `${BACKEND_URL}/api/admins/candidate/${id}`,
          {
            method: "GET",
            headers: {
        ...
        setImagePreview(data.image ? `${BACKEND_URL}${data.image}` : null);
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${BACKEND_URL}/api/admins/update-candidate`,
        {
          method: "PUT",
          body: data,
        }
      ...
      navigate("/admin/view-candidates");
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="min-h-screen flex">
      ...
      >
        ...
        {!loading && (
          <motion.div
            className="w-full max-w-lg bg-white p-8 rounded-xl shadow-lg mx-auto"
            variants={itemVariants}
          >
            <form onSubmit={handleSubmit}>
              ...
              >
                <PersonAdd />
                {loading ? "Updating Candidate..." : "Update Candidate"}
              </motion.button>
            </form>
          </motion.div>
        )}
